...
// TrimmedPath 选择 n 层路径作为短路径
func (l *GlobalLogger) TrimmedPath(f string, n int) string {
...
func (l *GlobalLogger) logFormatter(values ...interface{}) (messages []interface{}) {
	if len(values) > 1 {
		...
		info := SqlInfo{
			FileWithLine: l.TrimmedPath(fmt.Sprintf("%s", values[1]), 3),
		}
		if level == "sql" {
			// duration
