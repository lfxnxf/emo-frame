...
var packetPool = sync.Pool{New: func() interface{} { return &RPCPacket{} }}

func GetRPCPacket() *RPCPacket {
	p := packetPool.Get().(*RPCPacket)
	p.reset()
	return p
}
...
func (rp *RPCPacket) reset() {
	rp.ID = 0
	rp.Code = 0
	for _, v := range rp.Header {
		v.Key = v.Key[0:0]
		v.Value = v.Value[0:0]
	}
	rp.Payload = rp.Payload[0:0]
	rp.Tp = 0
	rp.Flags = 0
	rp.T = 0
}
